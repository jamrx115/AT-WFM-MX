$(function(){
	...
	function nodo(nodo) {
		...
		$('#new_nodo_' + nodo).on('click', function(){
			...
			tipo_conector.find('.control-label').text("Componente:");
			tipo_conector.find('.controls').append($("<select class='form-control' name='tipo_conector_" + nodo + "_" + data_rows + "'><option selected>AP</option><option>Módem</option></select>"));
			...
			conector.find('.control-label').text("MAC:");
			conector.find('.controls').append($("<input class='form-control' name='conector_" + nodo + "_" + data_rows + "' type='text'>"));
